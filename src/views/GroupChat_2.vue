<template>
  <div class="chat-view">
    <div class="chat-header">
      ...
      <div class="chat-avatar" @click="showGroupInfo = true">
    ...
    <div class="chat-messages" ref="messagesContainer">
      ...
      >
        <div class="system-message-content">
          <div class="system-message-text" >{{ msg.content }}</div>
          <div class="system-message-time">{{ formatTime(msg.time) }}</div>
        </div>
      </div>
    ...
    <GroupInfo
      ...
      @update:visible="showGroupInfo = $event"
      @refresh-system-messages="handleRefreshSystemMessages"
    />
  </div>
</template>
...
import GroupInfo from '../components/GroupInfo_2.vue';
...
export default {
  ...
  setup() {
    ...
    const displayGroupName = computed(() => {
      return targetGroup.value?.group_name  || '未知群聊';
    });
    ...
    const loadSystemMessages = async () => {
      if (!targetGroup.value?.group_id) return;
      
      try {
        const res = await axios.get(
          `${API_CONFIG.BASE_URL}/group/${targetGroup.value?.group_id}/system-message`,
          { headers: { Authorization: `Bearer ${store.state.token}` } }
        );
        ...
        console.log("打印群消息系统提示： " , res.data)
      } catch (error) {
    ...
    const addSystemMessage = (msg) => {
      const exists = systemMessages.value.some(m => m.id === msg.id);
      if (!exists) {
    ...
    const findSenderName = (msg) => {
      if (msg.fromMe) return '我';
      return msg.from_username || `用户${msg.fromId}`;
    };
    ...
    const handleRefreshSystemMessages = async () => {
            await loadSystemMessages(); // 重新加载系统消息
            };
